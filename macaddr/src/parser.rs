...
impl<const N: usize, const N2: usize> MacParser<N, N2> {
    ...
    #[inline]
    const fn val(v: u8) -> Result<u8, ParseError> {
        match v {
            b'A'..=b'F' => Ok(10 + (v - b'A')),
            b'a'..=b'f' => Ok(10 + (v - b'a')),
            b'0'..=b'9' => Ok(v - b'0'),
    ...
    #[inline]
    const fn dval(b1: u8, b2: u8) -> Result<u8, ParseError> {
        // ? is not available in const
        match (Self::val(b1), Self::val(b2)) {
    ...
    const fn from_hex(s: &[u8]) -> Result<[u8; N], ParseError> {
        ...
        let mut i = 0;
        while i < N {
            result[i] = match Self::dval(s[2 * i], s[2 * i + 1]) {
                Ok(v) => v,
                Err(e) => return Err(e),
            };
            i += 1;
